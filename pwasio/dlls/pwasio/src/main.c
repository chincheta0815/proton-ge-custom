...
static HRESULT WINAPI QueryInterface(LPCLASSFACTORY UNUSED(_lpclassfactory), REFIID UNUSED(_refiid), LPVOID *ptr) {
  WINE_TRACE("\n");
...
static HRESULT WINAPI LockServer(LPCLASSFACTORY UNUSED(_lpclassfactory), BOOL UNUSED(_bool)) {
  WINE_TRACE("\n");
  return S_OK;
}
...
BOOL WINAPI DllMain(HINSTANCE hinst, DWORD reason, LPVOID UNUSED(_lpvoid)) {
  WINE_TRACE("%d\n", reason);
...
#define CHK(call)                                                              \
  ...
  CHK(RegSetValueExA(ips32, nullptr, 0, REG_STR("pwasio.dll")));
  CHK(RegSetValueExA(ips32, "ThreadingModel", 0, REG_STR("Apartment")));

  CHK(RegCreateKeyExA(HKEY_LOCAL_MACHINE, "Software\\ASIO\\pwasio", 0, nullptr, 0, KEY_WRITE,
                      nullptr, &driver, 0));
...
#define CHK(call)                                                              \
  ...
  CHK(RegDeleteTreeA(HKEY_LOCAL_MACHINE, "Software\\ASIO\\pwasio"));
  CHK(RegDeleteTreeA(HKEY_CURRENT_USER, "Software\\ASIO\\pwasio"));
#undef CHK
