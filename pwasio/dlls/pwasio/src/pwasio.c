...
static struct spa_thread *_create(void *_data, const struct spa_dict UNUSED(*_spa_dict),
...
static int _join(void *_data, struct spa_thread UNUSED(*_spath_tread), void **retval) {
  struct thread *t = _data;
  if (!t->handle)
    return -1;
...
static int _get_rt_range(void UNUSED(*_data), const struct spa_dict UNUSED(*_spa_dict), int *min, int *max) {
  *min = THREAD_PRIORITY_NORMAL;
  *max = THREAD_PRIORITY_TIME_CRITICAL;
  return 0;
}
static int _acquire_rt(void *_data, struct spa_thread UNUSED(*_spa_thread), int priority) {
  struct thread *t = _data;
  int err = 0;
...
static int _drop_rt(void *_data, struct spa_thread UNUSED(*_spa_thread)) {
  struct thread *t = _data;
  if (t->priority) {
...
struct pwasio {
  ...
  HINSTANCE hinst;
#ifndef __PROTON_7_BUILD__
  HANDLE panel;
#endif /* __PROTON_7_BUILD__ */
  _Atomic HWND dialog;
};
...
static void _error(void *_data, uint32_t id, int UNUSED(_int), int res, const char UNUSED(*_char)) {
  struct context *context = _data;
  if (id == PW_ID_CORE) {
    if (res == -ENOENT)
      return;
...
int _property(void *_data, uint32_t UNUSED(_unint32), const char *key, const char UNUSED(*_char),
              const char *value) {
...
static void _global(void *_data, uint32_t id, uint32_t UNUSED(_uint32), const char *type,
...
STDMETHODIMP_(LONG32) Init(struct asio *_data, void UNUSED(*_void)) {
  ...
  *context->nodes = (typeof(*context->nodes)){
  ...
  HKEY key = nullptr;
  if (RegCreateKeyEx(HKEY_CURRENT_USER, "Software\\ASIO\\pwasio", 0, nullptr, 0,
...
HRESULT WINAPI CreateInstance(LPCLASSFACTORY _data, LPUNKNOWN outer, REFIID UNUSED(_refiid),
                              LPVOID *ptr) {
  WINE_TRACE("\n");
  if (outer)
