...
#define CHK(call)                                                              \
  ...
  CHK(RegCreateKeyExA(HKEY_CLASSES_ROOT, "CLSID", 0, NULL, 0, KEY_WRITE,
                      NULL, &class, NULL));

  StringFromGUID2(&class_id, wstr, 39);
  CHK(RegCreateKeyExW(class, wstr, 0, NULL, 0, KEY_WRITE, NULL, &clsid,
                      NULL));

  CHK(RegSetValueExA(clsid, NULL, 0, REG_STR("pwasio Object")));

  CHK(RegCreateKeyExA(clsid, "InProcServer32", 0, NULL, 0, KEY_WRITE,
                      NULL, &ips32, NULL));

  CHK(RegSetValueExA(ips32, NULL, 0, REG_STR(LIB_NAME)));
  CHK(RegSetValueExA(ips32, "ThreadingModel", 0, REG_STR("Apartment")));

  CHK(RegCreateKeyExA(HKEY_LOCAL_MACHINE, L"Software\\ASIO\\pwasio", 0, NULL, 0, KEY_WRITE,
                      NULL, &driver, 0));
...
#define CHK(call)                                                              \
  ...
  CHK(RegDeleteTreeA(HKEY_LOCAL_MACHINE, L"Software\\ASIO\\pwasio"));
  CHK(RegDeleteTreeA(HKEY_CURRENT_USER, L"Software\\ASIO\\pwasio"));
#undef CHK
